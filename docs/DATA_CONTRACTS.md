...
| `POST /api/signals/:id/investigate`, `GET /api/investigations/:id` | Start and watch an investigation (idempotent per signal). The view's `usage` summary has `modelCalls`, `toolCalls`, `measuredModelCostUsd` (only what a provider reported: billed amounts, or reported tokens at list price), `upperBoundModelCostUsd` (calls whose charge is unknown, carried at the price cap), `budgetSpentUsd` (what the governor counted against the ceiling: every basis), `costBases`, `costBasis` (the first model call's basis) and `routedModels`. `budget` beside it is today's configuration; `investigation.budgetAtStart` is what that run was held to, and `investigation.gateAttempts` is every `GateResult` the run produced, oldest first (both null for runs stored before they were kept). |
| `GET /api/investigations[?limit=]` | Every run, newest first, without timelines: `id`, `signalId`, `symbol`, `status`, `stopReason`, `startedAt`, `finishedAt`, `briefId`, `gate` (`decision`, `confidenceCap`; null if no draft was judged), `draftsJudged`, and the same `usage` summary as the single view. |
| `GET /api/investigations/:id/chain` | The on-chain part of a rebase run as numbers: `chain` is null until a read exists, else `network`, `token`, `symbol`, `issuer` (`multiplierOld`, `multiplierNew` as the issuer's decimal strings, `effectiveTimeUtc`), `reads[]` (`key` BEFORE / AFTER / HEAD, `evidenceId`, `blockNumber`, `blockTime`, `multiplier` as a decimal string, `mode`), `activation` (`evidenceId`, `blockNumber`, `blockTime`, `mode`; null when none was found) and `activationSearched`. Every value is already public in the timeline's evidence summaries; nothing comes from the Brief. |
| `GET /api/desk/status` | The unattended loop: `enabled`, `intervalMinutes`, `maxInvestigationsPerDay`, `investigationsLast24h`, `lastTick` (`at`, `action` STARTED or IDLE, `reason`, `detail`; process memory, so null after a restart until the first tick), `nextTickAt`, `dailySpendCeilingUsd`, and `liveWindowHours` (how far back the detector looks). |
| `GET /api/activity[?limit=]` | One feed of recorded events, newest first (default 50, at most 200): `kinds` lists the values of `kind` (`SIGNAL_DETECTED`, `SIGNAL_SUPERSEDED`, `INVESTIGATION_STARTED`, `GATE_DECISION`, `INVESTIGATION_PUBLISHED`, `INVESTIGATION_REJECTED`, `INVESTIGATION_STOPPED`, `QUOTE_ISSUED`, `ORDER_STATE`); each event has `at`, `kind`, `refId`, `symbol`, `summary`, `rail` (quote and order events only) and `state` (`ORDER_STATE` only). Summaries are assembled by code from stored fields. They never carry model text, a gate finding's detail, or what a facilitator or an RPC said about a payment. |
