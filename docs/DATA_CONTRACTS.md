...
| Route | Purpose |
| --- | --- |
| `GET /api/health` | Data mode, synthesis readiness (`provider`, `model`, `ready`, `detail`; for OpenRouter the detail states the cost tier and the price cap), rail readiness, budget, SDK versions. |
...
| `GET /api/v1/catalog` | Machine-readable list of what is for sale, for agents. Withdrawn Briefs are omitted. |
