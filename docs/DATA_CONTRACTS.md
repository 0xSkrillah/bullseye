...
| Route | Purpose |
| --- | --- |
| `GET /api/health` | Data mode, synthesis readiness (`provider`, `model`, `ready`, `detail`; for OpenRouter the detail states the cost tier and the price cap), rail readiness, budget, `autoDesk` (`enabled`, and when enabled `intervalMinutes` and `maxInvestigationsPerDay`), SDK versions. |
