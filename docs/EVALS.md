...
| Sign | Signed values are compared, never magnitudes. A minus sign binds only to a negative value; a hyphen-minus, U+2212, and the hyphens and dashes typography puts in its place (U+2010 to U+2013, U+FE63, U+FF0D) are all read as one when they stand directly against the digit, so "–0.67%" is negative and "10–12" is not. A figure written without a minus sign binds to a negative value only on a `signed` field, and only when a direction word stands within six words of it in the same sentence, at least as near as any word of the opposite direction. Direction words are read by kind: for an offset or a lag in seconds, words of time ("before", "earlier", "prior", "ahead" against "after", "later"); for a change or a surplus, words of size ("fell", "lower", "decrease", "reduction", "deficit", "shortfall" and the like against "rose", "higher", "increase", "gain", "surplus"). So "the prior multiplier" says nothing about a percentage. "up" and "down" are not read. A positive value of a signed field does not bind when a negative word of its kind is the nearer, and the rejection names the word that was read. On every other field the words around a figure are not read for sign. |
...
(in a claim, only when declared). A figure that stands directly after a word naming a value
("version 2 of the record", "block 70922364 is the first record") is that value and never a
count. A unit word, a preposition, "is", "was", "as", "when", "while", "until", "across" or the
end of a clause between the number and the noun stops the search, so in "3 shares and 5 checks passed" the 3 is a share
