...
`npm test` (Vitest, no network, no keys) runs 150 API tests in 15 files under `apps/api/test/`
...
| `prompts.test.ts` | The writing prompt names every key of the schema the provider is sent, so the Brief's shape does not depend on the routed endpoint enforcing `response_format`; it carries no sample values a model could copy; a revision repeats the shape only when the rejection was about the shape. Written after the deployed desk's first run was rejected twice on `SCHEMA`. |
