...
export function usePoll<T>(fn: () => Promise<T>, ms: number, active = true, deps: unknown[] = []): { data: T | null; error: string | null } {
  ...
  useEffect(() => {
    ...
    let stop = false;
    const tick = async () => {
      try { const d = await fn(); if (!stop) { setData(d); setError(null); } }
